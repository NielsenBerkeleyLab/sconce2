...
AllInd0TrParam2DegPolyHMM::AllInd0TrParam2DegPolyHMM(std::vector<DepthPair*>* depths, std::vector<std::string>* sampleList, gsl_vector* fixedParams, int maxPloidy) : AllInd0TrParam2DegPolyHMM(depths, sampleList, fixedParams, maxPloidy, 0, 3, 0, depths->size(), depths->size() * 1) { // 0 shared transition params to est, 3 fixed transition param (alpha/beta/gamma), 0 fixed libs, n HMMs, n * 1 branches
}
...
/*
 * same as AllInd3TrParam2DegPolyHMM::setAllTransition but skips over beta/gamma terms, and only takes branches
 ...
 * assumes params = [lib, t]
 */
void AllInd0TrParam2DegPolyHMM::setParamsToEstFromIthHMM(gsl_vector* params, int hmmIdx) {
  gsl_vector_set(this->paramsToEst, this->LIB_SIZE_SCALING_FACTOR_START_IDX + hmmIdx, gsl_vector_get(params, 0)); // lib
  gsl_vector_set(this->paramsToEst, this->BRANCH_LENGTH_START_IDX + hmmIdx, gsl_vector_get(params, 1)); // t
}
/*
 * given params from hmmIdx'th HMM, save into this class's fixedParams. Does not save into the hmm itself.
 * assumes params = [alpha, beta, gamma]
 */
void AllInd0TrParam2DegPolyHMM::setFixedParamsFromIthHMM(gsl_vector* params, int hmmIdx) {
  ...
  gsl_vector_set(this->fixedParams, this->FIXED_TRANSITION_PROB_START_IDX + 2, gsl_vector_get(params, 2)); // gamma
}
 ...
 */
AllInd0TrParam2DegPolyHMM* AllInd0TrParam2DegPolyHMM::bfgs(gsl_vector* initGuess, int maxIters, bool verbose, bool debug) {
  ...
  for(unsigned int hmmIdx = 0; hmmIdx < this->hmmVec->size(); hmmIdx++) {
    ...
    HMM* currBestHMM = (HMM*)(*this->hmmVec)[hmmIdx]->bfgs(currInitGuess, maxIters, verbose, debug);
