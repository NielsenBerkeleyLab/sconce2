...
//}
AllPairs0TrParam2DegPolyHMM::AllPairs0TrParam2DegPolyHMM(std::vector<DepthPair*>* depths, std::vector<std::string>* sampleList, gsl_vector* fixedParams, int maxPloidy, int numFixedLibs, int numPairs, int numBranchesToEst) : AllPairs3TrParam2DegPolyHMM(depths, sampleList, fixedParams, maxPloidy, 0, 3, numFixedLibs, numPairs, numBranchesToEst) { // 0 numSharedTrParamsToEst, 3 numFixedTrParams (alpha/beta/gamma), passed numFixedLibs, passed numPairs, passed numBranchesToEst
...
/*
 * copies transition parameters only
 */
void AllPairs0TrParam2DegPolyHMM::setFixedTrParams(gsl_vector* params) {
  for(unsigned int i = 0; i < params->size; i++) {
    gsl_vector_set(this->fixedParams, this->FIXED_TRANSITION_PROB_START_IDX + 1 + i, gsl_vector_get(params, i));
  }
...
/*
 * same as AllPairs3TrParam2DegPolyHMM::setAllTransition but skips over beta/gamma terms, and only takes branches
...
AllPairs0TrParam2DegPolyHMM* AllPairs0TrParam2DegPolyHMM::bfgs(gsl_vector* initGuess, int maxIters, bool verbose, bool debug) {
  AllPairs0TrParam2DegPolyHMM* bestGuessOptim = this;
