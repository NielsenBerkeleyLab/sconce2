...
/*
 * This file is a main function to run sconce2.
 * First it runs sconce to estimate library sizes, beta, lambda, and tree branch lengths independently for all cells.
 * Then, lib sizes are fixed, beta and lambda are fixed at the median of all of their estimates.
 * Then, BFGS is used to estimate tree branch lengths across all pairs of cells.
 *
...
int main(int argc, char** argv) {
  ...
  IndThenPairs2Stages3TrParam2DegPolyHMM* indThenPairsHMM = new IndThenPairs2Stages3TrParam2DegPolyHMM(depthsVec, sampleList, maxKploid, runIndBFGS, numPairsStage2, verbose, debug, !forwardDiff); // flip forwardDiff flag to centralDiff
  ...
  if(verbose) {
    std::cout << "######## DONE WITH INDEPENDENT CELL ESTIMATION. optimizedIndParamsToEst IS ########" << std::endl;
  ...
  if(!estStage2Libs) {
    stage2FixedParams = gsl_vector_alloc(depthsVec->size() + 3); // all libs + alpha/beta/lambda are fixed
    ...
    stage2FixedParams = gsl_vector_alloc(3); // alpha/beta/lambda are fixed
