...
IndThenPairs2Stages3TrParam2DegPolyHMM::IndThenPairs2Stages3TrParam2DegPolyHMM(std::vector<DepthPair*>* depths, std::vector<std::string>* sampleList, int maxPloidy, bool runIndBFGS, int numPairsStage2, bool verbose, bool gradientDebug, bool centralDiff) :
                                        ...
                                        NUM_SHARED_TRANSITION_PARAMS_TO_EST(2), // beta, gamma
...
int IndThenPairs2Stages3TrParam2DegPolyHMM::getIndInitGuessSize() const {
  // initGuess for the independent stages should be shared across all cells (ie one est for [beta, gamma, t])
 ...
 * beta is trParamIdx 0, gamma is trParamIdx1
 ...
 * entries are [median(beta), median(gamma)]
 */
 ...
 * entries are [libA, libB, ..., alpha, median(beta), median(gamma), tA, tB, ...]
 */
 ...
 *
 * stage2FixedParams (fixed params for stage 2): [[libs,] alpha, beta, gamma], from indEstParams
 * stage2InitGuess (starting points for [libs and] branches): [[libs,] {t1,t2,t3}_AB, {t1,t2,t3}_AC,...]
 * indEstParams (estimated params from the independent stage; expected from getIndOptimParamsToEstSummary): [libA, libB, ..., alpha, median(beta), median(gamma), tA, tB, ...]
...
void IndThenPairs2Stages3TrParam2DegPolyHMM::copyStage1EstsIntoStage2FixedParams(gsl_vector* stage2FixedParams, gsl_vector* stage2InitGuess, gsl_vector* indEstParams, int numStage2LibsToFix) {
  ...
  stage2FixedParamsIdx++;
  gsl_vector_set(stage2FixedParams, stage2FixedParamsIdx, gsl_vector_get(indEstParams, this->NUM_CELLS + 2));// gamma
  stage2FixedParamsIdx++;
 ...
 * assumes each file is in order of [lib, alpha, beta, gamma, t]
 * entries are [libA, libB, ..., alpha, median(beta), median(gamma), tA, tB, ...]
 */
