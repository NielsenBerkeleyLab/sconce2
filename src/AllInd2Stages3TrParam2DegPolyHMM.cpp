...
int AllInd2Stages3TrParam2DegPolyHMM::getInitGuessSize() const {
  return this->bfgsAllInd->getNumParamsToEst(); // every param to be estimated has an entry here
}
...
int AllInd2Stages3TrParam2DegPolyHMM::getBWInitGuessSize() const {
  ...
  return this->bwAllInd->getNumParamsToEst(); // every param to be estimated has an entry here
}
 ...
 * values of lib, beta, gamma, t
 */
void AllInd2Stages3TrParam2DegPolyHMM::setSharedInitGuess(gsl_vector* initGuess, double lib, double beta, double gamma, double t) const {
  int initGuessIdx = 0;
  // lib scaling factors
  ...
  initGuessIdx++;
  gsl_vector_set(initGuess, initGuessIdx, gamma);
  initGuessIdx++;
