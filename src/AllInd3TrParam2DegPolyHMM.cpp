...
AllInd3TrParam2DegPolyHMM::AllInd3TrParam2DegPolyHMM(std::vector<DepthPair*>* depths, std::vector<std::string>* sampleList, int maxPloidy) : AllInd3TrParam2DegPolyHMM(depths, sampleList, nullptr, maxPloidy, 2, 1, 0, depths->size(), depths->size() * 1) { // 2 shared transition params to est (beta/gamma), 1 fixed transition param (alpha), 0 fixed libs, n numHMMs, n * 1 branches
}
...
void AllInd3TrParam2DegPolyHMM::makeHMMs() {
  ...
  gsl_vector_set(transitionParams, 1, 500); // gamma
 ...
 * assumes params = [lib, beta, gamma, t]
 */
void AllInd3TrParam2DegPolyHMM::setParamsToEstFromIthHMM(gsl_vector* params, int hmmIdx) {
  ...
  gsl_vector_set(this->paramsToEst, this->SHARED_TRANSITION_PROB_START_IDX + 1, gsl_vector_get(params, 2)); // gamma
...
void AllInd3TrParam2DegPolyHMM::convertProbToParam(gsl_vector* dest, const gsl_vector* src) const {
  ...
  gsl_vector_set(dest, this->SHARED_TRANSITION_PROB_START_IDX + 1, log(gamma / 1.0e6)); // set z
...
void AllInd3TrParam2DegPolyHMM::convertParamToProb(gsl_vector* dest, const gsl_vector* src) const {
  ...
  gsl_vector_set(dest, this->SHARED_TRANSITION_PROB_START_IDX + 1, exp(z) * 1e6); // gamma
  ...
  if(this->baumWelchParamResults != nullptr && iter < (int) this->baumWelchParamResults->size()) {
    ...
    if(numTotalRuns < 0) {
      ...
      if(numTotalRuns == -1) {
        ...
        gsl_vector_set(initGuess, this->SHARED_TRANSITION_PROB_START_IDX + 1, 500); // gamma
        ...
        gsl_vector_set(initGuess, this->SHARED_TRANSITION_PROB_START_IDX + 1, 100); // gamma
        ...
        gsl_vector_set(initGuess, this->SHARED_TRANSITION_PROB_START_IDX + 1, 1000); // gamma
...
void AllInd3TrParam2DegPolyHMM::baumWelchLeastSquares_convertProbToParam(gsl_vector* dest, const gsl_vector* src) const {
  ...
  gsl_vector_set(dest, this->SHARED_TRANSITION_PROB_START_IDX - this->NUM_LIBS_TO_EST + 1, log(gamma / 1.0e6)); // set z
...
void AllInd3TrParam2DegPolyHMM::baumWelchLeastSquares_convertParamToProb(gsl_vector* dest, const gsl_vector* src) const {
  ...
  gsl_vector_set(dest, this->SHARED_TRANSITION_PROB_START_IDX - this->NUM_LIBS_TO_EST + 1, exp(z) * 1e6); // gamma
