...
DepthPair::DepthPair(std::string diploidFilename, std::string tumorFilename) {
  ...
  this->chrToDiploidDepthMap = new std::unordered_map<std::string, std::vector<double>*>(); // chr:vector of mean diploid depths
  this->chrToDiploidVarMap = new std::unordered_map<std::string, std::vector<double>*>(); // chr:vector of diploid variances
  this->chrToTumorDepthMap = new std::unordered_map<std::string, std::vector<double>*>(); // chr:vector of tumor depths
...
/*
 * function to create a chr:vec of start coords map for all windows, and a chr:lineNum map. useful for identifying which windows a mutation falls in, as well as that window's absolute line number from depth file (ie easy debugging lookup)
 */
std::pair<std::unordered_map<std::string, std::vector<long long int>*>*,
          std::unordered_map<std::string, std::vector<int>*>*>*
          DepthPair::createChrWindowIdxLineNumMaps() {
  std::unordered_map<std::string, std::vector<long long int>*>* chrWindowStartMap = new std::unordered_map<std::string, std::vector<long long int>*>(this->chrVec->size());

  std::unordered_map<std::string, std::vector<int>*>* chrWindowLineNumMap = new std::unordered_map<std::string, std::vector<int>*>(this->chrVec->size());

  std::vector<std::string>* chrRegionVec = nullptr;
  std::vector<long long int>* windowStartVec = nullptr;
  std::vector<int>* windowIdxVec = nullptr;
  std::string currChr;
  std::string region;
  std::vector<std::string> regionParts;
  long long int start = -1;
  int lineNum = 1; // line number from windows file
  // for each chr
  for(unsigned int chrIdx = 0; chrIdx < this->chrVec->size(); chrIdx++) {
    currChr = (*this->chrVec)[chrIdx];
    chrRegionVec = (*this->regions)[currChr];
    windowStartVec = new std::vector<long long int>(chrRegionVec->size());
    windowIdxVec = new std::vector<int>(chrRegionVec->size());

    // for each window
    for(unsigned int regionIdx = 0; regionIdx < chrRegionVec->size(); regionIdx++) {
      // parse region key (chr:start-end)
      region = (*chrRegionVec)[regionIdx];
      boost::split(regionParts, region, boost::is_any_of(":-"));
      start = strtoll(regionParts[1].c_str(), nullptr, 10);

      // store start and lineNum
      (*windowStartVec)[regionIdx] = start;
      (*windowIdxVec)[regionIdx] = lineNum;
      lineNum++;
    }
    (*chrWindowStartMap)[currChr] = windowStartVec;
    (*chrWindowLineNumMap)[currChr] = windowIdxVec;
  } // end chr

  std::pair<std::unordered_map<std::string, std::vector<long long int>*>*, std::unordered_map<std::string, std::vector<int>*>*>* chrWindowIdxLineNumMaps = new std::pair<std::unordered_map<std::string, std::vector<long long int>*>*, std::unordered_map<std::string, std::vector<int>*>*>(chrWindowStartMap, chrWindowLineNumMap);
  return chrWindowIdxLineNumMaps;
}
