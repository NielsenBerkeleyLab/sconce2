 ...
 * alpha is fixed, and beta and gamma are shared between pairs.
 * Library sizes are independently estimated, as are branch lengths.
 *
 * Beta and gamma are optionally estimated via baum welch + least squares,
...
class AllInd2Stages3TrParam2DegPolyHMM {
  ...
  public:
    // constants
    const int NUM_CELLS;
    const int NUM_SHARED_TRANSITION_PARAMS_TO_EST; // ie. beta, gamma. Does not count branch lengths or lib size scaling factors
    ...
    void setUpBaumWelch();
    void setUpAllIndBFGS(bool fixLib, bool estTrParamsBFGS, bool centralDiff, gsl_vector* fixedParams);
    ...
    void setBWAllMeanVarianceFn(gsl_vector* meanVarianceCoefVec);
    void setBFGSAllMeanVarianceFn(gsl_vector* meanVarianceCoefVec);

    double getBWAllIndLogLikelihood();
    double getBFGSAllIndLogLikelihood();

    AllInd3TrParam2DegPolyHMM* bfgs(gsl_vector* initGuess, int maxIters, bool verbose = true, bool debug = false);
    AllInd3TrParam2DegPolyHMM* callBFGSNTimes(gsl_vector* initGuess, int numRuns, int maxIters, bool verbose = true, bool debug = false);
    void setInitGuessNthTime(gsl_vector* initGuess, int iter) const;
    void setSharedInitGuess(gsl_vector* initGuess, double lib, double beta, double gamma, double t) const;
    void setBaumWelchInitGuess(gsl_vector* initGuess, int numBWIters, int numLibStarts = 3, double libStartVal = 1.0, bool verbose = true, bool debug = false); // runs baum welch to find a good starting place for bfgs, saves into passed initGuess
    ...
    void saveParamEstimates(int cellIdx, std::string filename);
    void getParamsToEstFromFile(int cellIdx, std::string filename, int numExpectedLines, gsl_vector* meanVarianceCoefVec);
    bool hasHMMBeenReadFromFile();
    gsl_vector* getHMMParamsToEstReadFromFile();
