 ...
 * lengths (t1/t2/t3/t) are estimated for each HMM.
 *
 * Parameters are:
 *   - 3 rate parameters (alpha=P(adjacent CNA); beta=P(any CNA); gamma=P(event happens to both cells))
 *   - a library size scaling factor for each cell
 *   - branch lengths (3 branches between 2 cells, 1 scaling "branch" for 1 cell)
 *
