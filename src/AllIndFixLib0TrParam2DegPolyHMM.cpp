...
AllIndFixLib0TrParam2DegPolyHMM::AllIndFixLib0TrParam2DegPolyHMM(std::vector<DepthPair*>* depths, std::vector<std::string>* sampleList, gsl_vector* fixedParams, int maxPloidy) : AllIndFixLib0TrParam2DegPolyHMM(depths, sampleList, fixedParams, maxPloidy, 0, 3, depths->size(), depths->size(), depths->size() * 1) { // 0 shared transition params to est, 3 fixed transition param (alpha/beta/gamma), n fixed libs, n HMMs, n * 1 branches
}
...
void AllIndFixLib0TrParam2DegPolyHMM::makeHMMs(gsl_vector* meanVarianceCoefVec, gsl_vector* transitionParams) {
  ...
  for(unsigned int hmmIdx = 0; hmmIdx < this->depthsVec->size(); hmmIdx++) {
    ...
    currFixedParams = gsl_vector_alloc(1 + 3); // 1 lib, alpha/beta/gamma
    // set libs
    ...
    gsl_vector_set(currFixedParams, 3, gsl_vector_get(this->fixedParams, this->FIXED_TRANSITION_PROB_START_IDX + 2)); // gamma
 ...
 * assumes params = [t]
 */
void AllIndFixLib0TrParam2DegPolyHMM::setParamsToEstFromIthHMM(gsl_vector* params, int hmmIdx) {
  gsl_vector_set(this->paramsToEst, this->BRANCH_LENGTH_START_IDX + hmmIdx, gsl_vector_get(params, 0)); // t
}
/*
 * given params from hmmIdx'th HMM, save into this class's fixedParams. Does not save into the hmm itself.
 * assumes params = [lib, alpha, beta, gamma]
 */
void AllIndFixLib0TrParam2DegPolyHMM::setFixedParamsFromIthHMM(gsl_vector* params, int hmmIdx) {
  ...
  gsl_vector_set(this->fixedParams, this->FIXED_TRANSITION_PROB_START_IDX + 1, gsl_vector_get(params, 2)); // beta
  gsl_vector_set(this->fixedParams, this->FIXED_TRANSITION_PROB_START_IDX + 2, gsl_vector_get(params, 3)); // gamma
}
