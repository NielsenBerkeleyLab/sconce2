...
OneCellFixLib3TrParam2DegPolyHMM::OneCellFixLib3TrParam2DegPolyHMM(std::vector<DepthPair*>* depths, gsl_vector* fixedParams, int maxPloidy) : OneCellFixLib3TrParam2DegPolyHMM(depths, fixedParams, maxPloidy, 2, 1, 1, 1) { // 2 numTrParamsToEst (beta, gamma), 1 numFixedTrParams (alpha), 1 numFixedLibs, 1 numBranches
}
...
void OneCellFixLib3TrParam2DegPolyHMM::convertProbToParam(gsl_vector* dest, const gsl_vector* src) const {
  ...
  double gamma = gsl_vector_get(src, this->TRANSITION_PROB_START_IDX + 1);
  ...
  gsl_vector_set(dest, this->TRANSITION_PROB_START_IDX + 1, log(gamma)); // set z
...
void OneCellFixLib3TrParam2DegPolyHMM::convertParamToProb(gsl_vector* dest, const gsl_vector* src) const {
  ...
  gsl_vector_set(dest, this->TRANSITION_PROB_START_IDX + 1, exp(z)); // set gamma
