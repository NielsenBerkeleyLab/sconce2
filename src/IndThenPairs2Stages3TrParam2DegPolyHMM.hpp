...
/*
 * This class is for:
 * SCONCE as stage 0/1 to est [lib, beta, gamma, t] for each cell independently, including bw+ls and bfgs
 *
 * Because all parameters are relative, alpha is fixed in all cases.
 *
 * Then, stage 2 is all pairs of cells. libs = as is, beta = median(beta_A, beta_B,...), gamma = median(gamma_A, gamma_B,...).
