 ...
 * q_(i,j),(i',j') = mutation process forward in time of going from ploidy (i,j) to (i',j'), with rate gamma = P(event affects both bins)
 *              = { gamma * beta         if any same CNA > 1 (ie (i',j') \in {(i+n,j+n), or (i-n,j-n), n>1})
 *              = { gamma * (alpha+beta) if any same CNA = +-1 (ie (i',j') \in {(i+n,j+n), or (i-n,j-n), n=1})
 ...
 *
 * rateParams = [alpha, beta, gamma]
 */
void HMM::setRateMatrixQ(double alpha, double beta, double gamma) {
  ...
  for(unsigned int row = 0; row < this->rateMatrixQ->size1; row++) {
    ...
    for(unsigned int col = 0; col < this->rateMatrixQ->size2; col++) {
      ...
      // if move in tandem (include gamma term)
      // ex 0,1 -> 1,2  (+1)
      //    1,0 -> 2,1  (+1)
      //    2,3 -> 0,1  (-2)
      //    3,2 -> 1,0  (-2)
...
gsl_vector* HMM::createMeanVarianceCoefVec() {
  double intercept = 10.46385711652957084539;
...
double HMM::estLibScalingFactorsPosterior(int cellNum) {
  ...
  double scalingFactor = (T - nEps) / sum;
...
void HMM::saveParamEstimates(std::string filename) const {
  gsl_vector* writingBuffer = gsl_vector_alloc(this->paramsToEst->size + this->fixedParams->size); // libs + alpha/beta/gamma + t's
  int bufferIdx = 0;
...
double HMM::getParamsToEstFromFile(std::string filename, int numExpectedLinesPerFile) {
  ...
  gsl_vector* readingBuffer = gsl_vector_alloc(this->paramsToEst->size + this->fixedParams->size); // libs + alpha/beta/gamma + t's
