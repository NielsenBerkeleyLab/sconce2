...
AllPairs3TrParam2DegPolyHMM::AllPairs3TrParam2DegPolyHMM(std::vector<DepthPair*>* depths, std::vector<std::string>* sampleList, int maxPloidy) : AllPairs3TrParam2DegPolyHMM(depths, sampleList, nullptr, maxPloidy, 2, 1, 0, boost::math::binomial_coefficient<double>(depths->size(), 2), boost::math::binomial_coefficient<double>(depths->size(), 2) * 3) { // 2 shared transition params (beta/gamma), 1 fixed transition param (alpha), 0 fixed libs, n choose 2 pairs, n choose 2 pairs * 3 branches
 ...
 * assumes params = [lib0, lib1, beta, gamma, t1, t2, t3]
 */
void AllPairs3TrParam2DegPolyHMM::setParamsToEstFromIthHMM(gsl_vector* params, int hmmIdx) {
  ...
  gsl_vector_set(this->paramsToEst, this->SHARED_TRANSITION_PROB_START_IDX + 1, gsl_vector_get(params, 3)); // gamma
...
void AllPairs3TrParam2DegPolyHMM::convertProbToParam(gsl_vector* dest, const gsl_vector* src) const {
  ...
  double gamma = gsl_vector_get(src, this->SHARED_TRANSITION_PROB_START_IDX + 1);
  gsl_vector_set(dest, this->SHARED_TRANSITION_PROB_START_IDX + 0, log(beta)); // set y
  gsl_vector_set(dest, this->SHARED_TRANSITION_PROB_START_IDX + 1, log(gamma)); // set z
...
void AllPairs3TrParam2DegPolyHMM::convertParamToProb(gsl_vector* dest, const gsl_vector* src) const {
  ...
  gsl_vector_set(dest, this->SHARED_TRANSITION_PROB_START_IDX + 1, exp(z)); // gamma
...
AllPairs3TrParam2DegPolyHMM* AllPairs3TrParam2DegPolyHMM::bfgs(gsl_vector* initGuess, int maxIters, bool verbose, bool debug) {
  AllPairs3TrParam2DegPolyHMM* bestGuessOptim = this;
...
void AllPairs3TrParam2DegPolyHMM::setInitGuessNthTime(gsl_vector* initGuess, int iter, int numTotalRuns) const {
  ...
  if(iter == 0) {
    ...
    gsl_vector_set(initGuess, this->SHARED_TRANSITION_PROB_START_IDX + 1, 0.075); // gamma
    ...
    gsl_vector_set(initGuess, this->SHARED_TRANSITION_PROB_START_IDX + 1, 0.025); // gamma
    ...
    gsl_vector_set(initGuess, this->SHARED_TRANSITION_PROB_START_IDX + 1, 0.09); // gamma
...
double AllPairs3TrParam2DegPolyHMM::baumWelchLeastSquares_calcSumSqResid(const gsl_vector* v) {
  ...
  double gamma = gsl_vector_get(probs, this->SHARED_TRANSITION_PROB_START_IDX - this->NUM_LIBS_TO_EST + 1);
  double currT1 = 0;
  double currT2 = 0;
  double currT3 = 0;
  ...
  for(unsigned int hmmIdx = 0; hmmIdx < this->hmmVec->size(); hmmIdx++) {
    ...
    gsl_vector_set(currHMMProbs, hmmTrIdx + 1, gamma);
...
void AllPairs3TrParam2DegPolyHMM::baumWelchLeastSquares_convertProbToParam(gsl_vector* dest, const gsl_vector* src) const {
  ...
  double gamma = gsl_vector_get(src, this->SHARED_TRANSITION_PROB_START_IDX - this->NUM_LIBS_TO_EST + 1);
  gsl_vector_set(dest, this->SHARED_TRANSITION_PROB_START_IDX - this->NUM_LIBS_TO_EST + 0, log(beta)); // set y
  gsl_vector_set(dest, this->SHARED_TRANSITION_PROB_START_IDX - this->NUM_LIBS_TO_EST + 1, log(gamma)); // set z
...
void AllPairs3TrParam2DegPolyHMM::baumWelchLeastSquares_convertParamToProb(gsl_vector* dest, const gsl_vector* src) const {
  ...
  gsl_vector_set(dest, this->SHARED_TRANSITION_PROB_START_IDX - this->NUM_LIBS_TO_EST + 1, exp(z)); // gamma
