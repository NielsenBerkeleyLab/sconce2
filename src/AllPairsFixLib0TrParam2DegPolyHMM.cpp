...
void AllPairsFixLib0TrParam2DegPolyHMM::makeOneHMMPair(int i, int j, bool preallocIntermediates) {
  ...
  gsl_vector* currFixedParams = gsl_vector_alloc(2 + 3); // 2 libs, alpha/beta/gamma
  // set libs
  ...
  gsl_vector_set(currFixedParams, 4, gsl_vector_get(this->fixedParams, this->FIXED_TRANSITION_PROB_START_IDX + 2)); // gamma
 ...
 * assumes params = [lib0, lib1, alpha, beta, gamma]
 */
void AllPairsFixLib0TrParam2DegPolyHMM::setFixedParamsFromIthHMM(gsl_vector* params, int hmmIdx) {
  ...
  gsl_vector_set(this->fixedParams, this->FIXED_TRANSITION_PROB_START_IDX + 2, gsl_vector_get(params, 4)); // gamma
}
 ...
 * This used to be an override, changed to be simple implementation to add filename for intermediate saving
 */
...
void AllPairsFixLib0TrParam2DegPolyHMM::callIndvHMMBFGS(int hmmIdx, gsl_vector* initGuess, std::string filename, int maxIters, bool verbose, bool debug) {
  ...
  if((*this->hmmVec)[hmmIdx] == nullptr) {
    ...
    this->getOnePairedOptimParamsToEstFromFile(hmmIdx, this->pairedEstimatesPath, 2 + 3 + 3); // 2 libs + alpha/beta/gamma + 3 branches per pair. includes a file.exists check
  }
