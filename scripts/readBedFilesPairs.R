...
calcAllSSE <- function(paramSet, key, outputFile, numCells, forceRecalc=F, inclAneu=F) {
...
makeSSEPlot <- function(toPlot, title) {  
  ...
  if(!is.na(title)) {
    p <- p + labs(x="method", y="SSE", colour="method", title=title) 
  } else {
...
makeDiminishingReturnsPlot <- function(toPlot, title) {  
  ...
  if(!is.na(title)) {
    p <- p + labs(x=expression(kappa*" = # cells summarized"), y="change in SSE", title=title)
  } else {
...
calcAllBreakpoints <- function(paramSet, key, outputFile, numCells, forceRecalc=F, inclAneu=F) {
  indvDat <- pairDat <- aneuDat <- NULL
  if(!forceRecalc && file.exists(paste0(outputFile, "_indv.txt")) && file.exists(paste0(outputFile, "_pairs.txt"))) {
    indvDat <- read.table(paste0(outputFile, "_indv.txt"), sep="\t", header=T)
    pairDat <- read.table(paste0(outputFile, "_pairs.txt"), sep="\t", header=T)
...
makeBreakpointPlot <- function(toPlot, title) {
  ...
  if(!is.na(title)) {
    p <- p + labs(x=expression(omega), y="breakpoint dist", title=title) 
  } else {
